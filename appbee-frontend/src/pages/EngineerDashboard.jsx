...
import { DollarSign, Briefcase, CheckCircle2, Play, UserCheck, FileText, ArrowRight, Clock } from 'lucide-react';
...
const EngineerDashboard = () => {
    ...
    const fetchMyTasks = async () => {
        ...
        try {
            ...
            // Sadece benim "submitted" veya "claimed" olduğum görevler
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredTasks.map((task, index) => {
                        ...
                        const isSubmitted = !isCompleted && task.submittedByMe;

                        // Kart Çerçeve Rengi
                        let borderClass = "border-blue-200 dark:border-slate-700 shadow-blue-500/5";
                        if (isCompleted) borderClass = "border-green-200 dark:border-green-900 shadow-green-500/5";
                        else if (isSubmitted) borderClass = "border-yellow-200 dark:border-yellow-900 shadow-yellow-500/5";
                        ...
                        return (
                            <motion.div
                                ...
                                className={clsx(
                                    "bg-white dark:bg-slate-900 rounded-2xl border shadow-xl transition-all group cursor-pointer relative overflow-hidden",
                                    borderClass
                                )}
                            >
                                {/* --- SAĞ ÜST ROZET (STATUS BADGE) --- */}
                                {isCompleted ? (
                                    <div className="absolute top-0 right-0 bg-green-100 text-green-700 text-[10px] font-bold px-3 py-1 rounded-bl-xl z-10 flex items-center gap-1">
                                        <CheckCircle2 size={12} /> TAMAMLANDI
                                    </div>
                                ) : isSubmitted ? (
                                    <div className="absolute top-0 right-0 bg-yellow-100 text-yellow-700 text-[10px] font-bold px-3 py-1 rounded-bl-xl z-10 flex items-center gap-1">
                                        <Clock size={12} /> İNCELENİYOR
                                    </div>
                                ) : (
                                ...
                                {/* --- AKSİYON BUTONU (ALT KISIM) --- */}
                                <div className="absolute bottom-0 left-0 right-0 bg-gray-50/80 dark:bg-slate-800/50 p-4 border-t border-gray-100 dark:border-slate-700 flex items-center justify-between backdrop-blur-sm">
                                    ...
                                    {isCompleted ? (
                                        ...
                                        <div className="flex items-center gap-2 text-yellow-600 font-bold text-sm bg-yellow-50 px-3 py-1.5 rounded-lg border border-yellow-200">
                                            <Clock size={16} /> Onay Bekliyor
                                        </div>
                                    ) : (
                                        <button className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-blue-700 transition-colors shadow-md shadow-blue-200 dark:shadow-none">
